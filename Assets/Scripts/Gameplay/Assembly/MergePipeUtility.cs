...
public static class MergePipeUtility
{
    private const string MergePipeName = "Merge Pipe";
    private const float CellSize = 0.1f;
    private const int GridSize = 99;

    public static bool IsMergePipePart(Part targetPart)
    {
        return targetPart != null
            && targetPart.partType == PartType.Pipe
            && !string.IsNullOrWhiteSpace(targetPart.partName)
            && string.Equals(targetPart.partName, MergePipeName, System.StringComparison.OrdinalIgnoreCase);
    }

    public static MergePipeState ResolveState(GameObject root)
    {
        if (root == null)
        {
            return null;
        ...
        MergePipeState state = root.GetComponent<MergePipeState>();
        if (state != null)
        {
            return state;
        }
        ...
        if (focus == null || !IsMergePipePart(focus.SourcePart))
        {
            return null;
        }

        return ComponentUtility.GetOrAddComponent<MergePipeState>(root);
    }

    public static bool TryResolveStateForCell(ArtificialSatellite ownerSatellite, Vector2Int cell, out MergePipeState state)
    {
        state = null;
        ...
        for (int i = 0; i < partFocuses.Length; i++)
        {
            ...
            if (partFocus == null || !IsMergePipePart(partFocus.SourcePart))
            {
                continue;
            }

            if (!TryResolveMergePipeCell(partFocus, out Vector2Int resolvedCell) || resolvedCell != cell)
            {
                continue;
            }
    ...
    private static bool TryResolveMergePipeCell(AssemblyPartFocus partFocus, out Vector2Int sourceCell)
    {
