...
[DefaultExecutionOrder(34004)]
[DisallowMultipleComponent]
public class SplitPipePresenter : FocusEventSubscriber
{
    ...
    private static readonly AssemblyPartPortLayout.PortSide[] SplitOutputLocalSides =
    {
    ...
    private static readonly Color OutputArrowSelectedColor = new Color(0.48f, 0.78f, 0.98f, 1f);
    ...
    private static SplitPipePresenter instance;
    ...
    private readonly HashSet<AssemblyPartPortLayout.PortSide> outputSidesBuffer = new HashSet<AssemblyPartPortLayout.PortSide>();
    private AssemblyPartFocus focusedSplitPipePart;
    private SplitPipeState focusedSplitPipeState;
    ...
    {
        focusedSplitPipePart = TryResolveFocusedSplitPipe(focused, out AssemblyPartFocus partFocus) ? partFocus : null;
        focusedSplitPipeState = focusedSplitPipePart != null ? SplitPipeUtility.ResolveState(focusedSplitPipePart.gameObject) : null;
        UpdatePanelState();
    }
    ...
    private void EnsurePanel()
    {
        ...
        titleLabel = CreateLabel(panelRect, "SplitPipeTitle", TitleText, new Vector2(0.03f, 0.93f), new Vector2(0.45f, 0.99f), 24f, FontStyles.Bold, TextAlignmentOptions.Left, WhiteText);
        statusLabel = CreateLabel(panelRect, "SplitPipeStatus", NoRouteText, new Vector2(0.03f, 0.88f), new Vector2(0.74f, 0.93f), 12f, FontStyles.Bold, TextAlignmentOptions.Left, WhiteText);
        Button closeButton = CreateButton(panelRect, "SplitPipeClose", "X", new Vector2(0.90f, 0.91f), new Vector2(0.985f, 0.995f), 18f, out _, CloseButtonColor, WhiteText, true);
        closeButton.onClick.AddListener(HandleCloseClicked);

        topDirectionView = CreateDirectionView(panelRect, "SplitPipeDirectionTop", new Vector2(0.42f, 0.81f), new Vector2(0.58f, 0.96f), 0f);
        leftDirectionView = CreateDirectionView(panelRect, "SplitPipeDirectionLeft", new Vector2(0.07f, 0.60f), new Vector2(0.25f, 0.78f), 90f);
        rightDirectionView = CreateDirectionView(panelRect, "SplitPipeDirectionRight", new Vector2(0.75f, 0.60f), new Vector2(0.93f, 0.78f), -90f);
        bottomDirectionView = CreateDirectionView(panelRect, "SplitPipeDirectionBottom", new Vector2(0.42f, 0.45f), new Vector2(0.58f, 0.60f), 180f);

        RectTransform transportCircle = CreateSection(panelRect, "SplitPipeTransportCircle", new Vector2(0.37f, 0.57f), new Vector2(0.63f, 0.81f), TransportCircleColor, true);
        transportItemLabel = CreateLabel(transportCircle, "SplitPipeTransportItem", DefaultTransportText, new Vector2(0.14f, 0.14f), new Vector2(0.86f, 0.86f), 28f, FontStyles.Bold, TextAlignmentOptions.Center, DarkText);

        recoverButton = CreateButton(panelRect, "SplitPipeRecoverButton", RecoverText, new Vector2(0.31f, 0.07f), new Vector2(0.69f, 0.44f), 24f, out recoverButtonBackground, ActionButtonColor, WhiteText, false);
        recoverButton.onClick.AddListener(HandleRecoverClicked);

        onButton = CreateButton(panelRect, "SplitPipeOnButton", "ON", new Vector2(0.80f, 0.25f), new Vector2(0.97f, 0.44f), 20f, out onButtonBackground, ActionButtonColor, WhiteText, false);
        onButton.onClick.AddListener(HandleOnClicked);

        offButton = CreateButton(panelRect, "SplitPipeOffButton", "OFF", new Vector2(0.80f, 0.07f), new Vector2(0.97f, 0.24f), 20f, out offButtonBackground, ActionButtonColor, WhiteText, false);
    ...
    private void HandleCloseClicked()
    {
        ...
        focusedSplitPipePart = null;
        focusedSplitPipeState = null;
    ...
    {
        return focusedSplitPipePart != null && focusedSplitPipePart.OwnerSatellite != null ? focusedSplitPipePart.OwnerSatellite.transform : null;
    }
    ...
    private void UpdatePanelState()
    {
        ...
        bool shouldShow = focusedSplitPipePart != null && focusedSplitPipeState != null;
    ...
    {
        if (focusedSplitPipePart == null || focusedSplitPipeState == null || !SplitPipeUtility.IsSplitPipePart(focusedSplitPipePart.SourcePart))
        {
            SetPanelVisible(false);
            return;
        }
    ...
    private void PopulateRouteContext()
    {
        ...
        if (focusedSplitPipePart == null || focusedSplitPipePart.OwnerSatellite == null)
        {
            ApplyRouteDisplay(transportName, currentStatus);
            return;
        }

        Vector2Int focusedCell = LocalPositionToGrid(focusedSplitPipePart.transform.localPosition);
        OutputPortProductionState[] states = focusedSplitPipePart.OwnerSatellite.GetComponentsInChildren<OutputPortProductionState>(true);
    ...
    {
        outputSidesBuffer.Clear();
        BuildWorldSplitPortSides(focusedSplitPipePart != null ? focusedSplitPipePart.transform.localRotation : Quaternion.identity, outputSidesBuffer, out AssemblyPartPortLayout.PortSide inputSide);
        bool hasSelectedOutput = TryResolveSelectedOutputSide(outputSidesBuffer, out AssemblyPartPortLayout.PortSide selectedOutputSide);
        ApplyDirectionView(topDirectionView, AssemblyPartPortLayout.PortSide.Top, inputSide, outputSidesBuffer, hasSelectedOutput, selectedOutputSide);
        ApplyDirectionView(rightDirectionView, AssemblyPartPortLayout.PortSide.Right, inputSide, outputSidesBuffer, hasSelectedOutput, selectedOutputSide);
        ApplyDirectionView(bottomDirectionView, AssemblyPartPortLayout.PortSide.Bottom, inputSide, outputSidesBuffer, hasSelectedOutput, selectedOutputSide);
        ApplyDirectionView(leftDirectionView, AssemblyPartPortLayout.PortSide.Left, inputSide, outputSidesBuffer, hasSelectedOutput, selectedOutputSide);
    }

    private void ApplyDirectionView(DirectionView view, AssemblyPartPortLayout.PortSide side, AssemblyPartPortLayout.PortSide inputSide, HashSet<AssemblyPartPortLayout.PortSide> outputSides, bool hasSelectedOutput, AssemblyPartPortLayout.PortSide selectedOutputSide)
    {
        ...
        bool isInput = side == inputSide;
        bool isOutput = outputSides != null && outputSides.Contains(side);
        ...
        bool isSelectedOutput = isOutput && hasSelectedOutput && side == selectedOutputSide;
        view.label.SetText(isInput ? InputText : OutputText);
        view.background.color = isInput ? InputArrowColor : (isSelectedOutput ? OutputArrowSelectedColor : OutputArrowColor);
        if (view.outline != null)
        {
            view.outline.effectColor = isSelectedOutput ? SelectedDirectionOutlineColor : DirectionOutlineColor;
            view.outline.effectDistance = isSelectedOutput ? new Vector2(2f, -2f) : new Vector2(1f, -1f);
        }

        view.container.localScale = isSelectedOutput ? new Vector3(1.06f, 1.06f, 1f) : Vector3.one;
    }

    private bool TryResolveSelectedOutputSide(HashSet<AssemblyPartPortLayout.PortSide> outputSides, out AssemblyPartPortLayout.PortSide selectedOutputSide)
    {
        selectedOutputSide = AssemblyPartPortLayout.PortSide.Right;
        if (focusedSplitPipePart == null || focusedSplitPipeState == null || focusedSplitPipePart.OwnerSatellite == null)
        {
            return false;
        }
        ...
        PipeConnectivityUtility.BuildPipePortMaps(focusedSplitPipePart.OwnerSatellite, inputDirectionsByCell, outputDirectionsByCell, splitStatesByCell, pipeCells);

        if (PipeConnectivityUtility.TryGetSelectedSplitOutputDirection(focusedCell, focusedSplitPipeState, inputDirectionsByCell, outputDirectionsByCell, pipeCells, out Vector2Int selectedDirection))
        {
            selectedOutputSide = ConvertDirectionToSide(selectedDirection);
            return outputSides == null || outputSides.Count <= 0 || outputSides.Contains(selectedOutputSide);
        }

        if (outputSides != null && outputSides.Count > 0)
        {
            List<AssemblyPartPortLayout.PortSide> previewSides = new List<AssemblyPartPortLayout.PortSide>(outputSides);
            return focusedSplitPipeState.TrySelectPreviewOutputSide(previewSides, out selectedOutputSide);
        }
    ...
    private static bool TryResolveFocusedSplitPipe(Transform focused, out AssemblyPartFocus partFocus)
    {
        partFocus = null;
        ...
        if (!SplitPipeUtility.IsSplitPipePart(partFocus.SourcePart))
        {
            partFocus = null;
            return false;
        }
    ...
    private static void BuildWorldSplitPortSides(Quaternion localRotation, HashSet<AssemblyPartPortLayout.PortSide> outputSides, out AssemblyPartPortLayout.PortSide inputSide)
    {
        outputSides?.Clear();
        ...
        for (int i = 0; i < SplitOutputLocalSides.Length; i++)
        {
            outputSides.Add(RotateSide(SplitOutputLocalSides[i], quarterTurns));
        }
    }
