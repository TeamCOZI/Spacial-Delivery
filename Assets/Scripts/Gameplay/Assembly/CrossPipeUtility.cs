...
public static class CrossPipeUtility
{
    private const string CrossPipeName = "Cross Pipe";
    private const float CellSize = 0.1f;
    private const int GridSize = 99;

    public static bool IsCrossPipePart(Part targetPart)
    {
        return targetPart != null
            && targetPart.partType == PartType.Pipe
            && !string.IsNullOrWhiteSpace(targetPart.partName)
            && string.Equals(targetPart.partName, CrossPipeName, System.StringComparison.OrdinalIgnoreCase);
    }
    ...
    public static Part ResolvePart()
    {
        ...
        {
            Part resolvedPart = partDb.GetPartByName(CrossPipeName);
        ...
        return Resources.Load<Part>("Parts/CrossPipe");
    }

    public static CrossPipeState ResolveState(GameObject root)
    {
        if (root == null)
        {
            return null;
        ...
        CrossPipeState state = root.GetComponent<CrossPipeState>();
        if (state != null)
        {
            return state;
        }
        ...
        if (focus == null || !IsCrossPipePart(focus.SourcePart))
        {
            return null;
        }

        return ComponentUtility.GetOrAddComponent<CrossPipeState>(root);
    }

    public static void BuildStateMap(ArtificialSatellite ownerSatellite, Dictionary<Vector2Int, CrossPipeState> statesByCell)
    {
        statesByCell?.Clear();
        if (ownerSatellite == null || statesByCell == null)
        {
            return;
        }

        CrossPipeState[] states = ownerSatellite.GetComponentsInChildren<CrossPipeState>(true);
        for (int i = 0; i < states.Length; i++)
        {
            CrossPipeState state = states[i];
            if (state == null || !TryResolveCrossPipeCell(state.gameObject, out Vector2Int sourceCell))
            {
                continue;
            }

            statesByCell[sourceCell] = state;
        }
    }

    public static bool TryResolveStateForCell(ArtificialSatellite ownerSatellite, Vector2Int cell, out CrossPipeState state)
    {
        state = null;
        ...
        for (int i = 0; i < states.Length; i++)
        {
            CrossPipeState candidate = states[i];
            if (candidate == null || !TryResolveCrossPipeCell(candidate.gameObject, out Vector2Int resolvedCell) || resolvedCell != cell)
            {
                continue;
            }
    ...
    private static bool TryResolveCrossPipeCell(GameObject root, out Vector2Int sourceCell)
    {
        ...
        if (partFocus == null || !IsCrossPipePart(partFocus.SourcePart))
        {
            return false;
        }
        ...
        if (layout == null)
        {
            layout = root.GetComponentInChildren<AssemblyPartPortLayout>(true);
        }
