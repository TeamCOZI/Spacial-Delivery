...
public class ScaledSpaceManager : MonoBehaviour
{
    ...
    private bool IsProxyTarget(MonoBehaviour behaviour)
    {
        ...
        if (behaviour.GetComponent<OrbitRevolution>() == null && behaviour.GetComponent<Star>() == null) return false;
        return true;
    }
    ...
    private ProxyEntry TryCreateProxyEntry(Transform source)
    {
        ...
        GameObject proxy = new GameObject(source.name + " [ScaledProxy]")
        {
            tag = source.tag,
        ...
        ScaledSpaceProxyTarget proxyTarget = proxy.AddComponent<ScaledSpaceProxyTarget>();
